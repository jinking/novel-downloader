...
> [!IMPORTANT]
> 1. **网络环境（科学上网 / 代理推荐）**：
>    部分高质量小说镜像站与聚合源部署在海外节点（Cloudflare CDN），境内直连网络可能会遭遇 DNS 污染或连接重置。**强烈建议在科学上网（开启全局/系统代理）环境下运行**，以确保所有多级节点与免验证搜索引擎均能毫秒级打通。
>
> 2. **AI 智能体模型选型建议（重要）**：
>    如果你将本项目接入 AI Agent（如 Claude Code、Antigravity、Cursor 等）：
>    - **推荐使用海外前沿大模型**（如 Claude 4.5 / 4.0 Sonnet & Opus、Gemini 3.0 Pro / Ultra、GPT-5 / o3 等），指令遵从度极高，具备强大的代码工程执行力与端到端自动化抓取调度能力。
>    - **慎用国内大模型**：国内模型普遍具备非常严格的商业版权合规与内容安全审查机制，在识别到小说书名、特定敏感风月词汇或防爬对抗任务时，极易直接触发拒答策略（Content Policy Invalidation），拒绝调用终端命令或提前阻断下载流程。
